...
namespace AvatarBuild.Editor
{
    ...
    internal class ElementBuilder : EditorWindow
    {
        ...
        private Vector2 meunPoint = Vector2.zero;
        private ElemetConfig elemetConfig;
        private ElementItem currentEditItem;
        ...
        public void OnGUI()
        {
            ...
            meunPoint = GUILayout.BeginScrollView(meunPoint, false, true, GUILayout.Width(205));
            this.OnDrawMenuList();
            GUILayout.EndScrollView();
            point = EditorGUILayout.BeginScrollView(point, EditorStyles.helpBox, GUILayout.Height(position.height - 25));
            OnDrawItems();
        ...
        private void OnDrawInspector()
        {
            ...
            GUILayout.Label(currentEditItem.path);
            GUILayout.EndHorizontal();
            GUILayout.BeginHorizontal();
            GUILayout.Label("Element Type:", GUILayout.Width(100));
            currentEditItem.element = (Element)EditorGUILayout.EnumPopup(currentEditItem.element, GUILayout.Width(200));
            GUILayout.EndHorizontal();
            currentEditItem.isNormal = GUILayout.Toggle(currentEditItem.isNormal, "Normal");

            UnityEngine.Object texture = AssetDatabase.LoadAssetAtPath(currentEditItem.path, typeof(GameObject));
            GUILayout.FlexibleSpace();
            if (GUILayout.Button("Upload Element"))
            {
                UploadConfig(currentEditItem);
            }
        ...
        private void OnDrawItems()
        {
            ...
            for (int i = 0; i < elemetConfig.elements.Count; i++)
            {
                ...
                Texture2D texture1 = AssetPreview.GetAssetPreview(texture);
                if (GUILayout.Button(texture1, GUILayout.Width(texture1.width), GUILayout.Height(texture1.height)))
                {
                    renderUtility = null;
                    currentEditItem = elemetConfig.elements[i];
                }
            }
        ...
        private void OnDrawMenuList()
        {
            ...
            for (int i = 0; i < names.Length; i++)
            {
                ...
                if (GUILayout.Button(names[i], current != (Element)i ? EditorStyles.label : EditorStyles.selectionRect, GUILayout.Width(185)))
                {
                    current = (Element)i;
                }
            }
        ...
        private void UploadConfig(params ElementItem[] elements)
        {
            ...
            void StartUpload()
            {
                ...
                UploadAsset(outputPath, element, exception =>
                {
                    ...
                    float progress = upload_state.Where(x => x == true).Count() / (float)upload_state.Length;
                    EditorUtility.DisplayProgressBar("Upload", "Upload element:" + element.path, progress);
                    if (exception != null)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            Debug.LogErrorFormat("{0}\n{1}", item.Key.path, item.Value.ToString());
                        }
        ...
        private void UploadAsset(string outputpath, ElementItem element, Action<Exception> callback)
        {
            ...
            WebService.UploadAssetResponse upload_modle_reponse = null;
            WebService.UploadAssetResponse upload_icon_response = null;
            WebService.UploadAssetResponse upload_texture_reponse = null;
            WebService.UploadAssetResponse upload_zip_response = null;

            void Runnable_UploadAssetZip()
            {
                bytes = GetZipDatas(element.path);
                WebService.RequestCreateFileData createAssetZip = new WebService.RequestCreateFileData(fileName + ".zip", bytes.GetMd5(), "zip", "2", bytes.Length);
                this.StartCoroutine(WebService.UploadAsset(elemetConfig.address, createAssetZip, bytes, (response, exception) =>
                 {
                     if (exception != null)
                     {
                         callback(exception);
            ...
            void Runnable_UploadAssetBundle()
            {
                ...
                WebService.RequestCreateFileData requestCreateModleFileData = new WebService.RequestCreateFileData(fileName + ".assetbundle", bytes.GetMd5(), "assetbundle", "2", bytes.Length);
                this.StartCoroutine(WebService.UploadAsset(elemetConfig.address, requestCreateModleFileData, bytes, (response, exception) =>
                 {
                     if (exception != null)
                     {
                         callback(exception);
            ...
            void Runnable_UploadElementIcon()
            {
                ...
                WebService.RequestCreateFileData requestCreateIconFileData = new WebService.RequestCreateFileData(fileName + "_icon.png", bytes.GetMd5(), "image/png", "2", bytes.Length);
                this.StartCoroutine(WebService.UploadAsset(elemetConfig.address, requestCreateIconFileData, bytes, (response, exception) =>
                 {
                     if (exception != null)
                     {
                         callback(exception);
            ...
            void Runnable_UploadDefaultTexture()
            {
                ...
                WebService.RequestCreateFileData requestCreateTextureFileData = new WebService.RequestCreateFileData(fileName + "_default.png", bytes.GetMd5(), "image/png", "2", bytes.Length);
                this.StartCoroutine(WebService.UploadAsset(elemetConfig.address, requestCreateTextureFileData, bytes, (response, exception) =>
                 {
                     if (exception != null)
                     {
                         callback(exception);
            ...
            void Runnable_AvatarElementData()
            {
                ...
                this.StartCoroutine(WebService.CreateElementRequest(elemetConfig.address, elementData, (response, exception) =>
                {
                    if (exception != null)
                    {
                        callback(exception);
        ...
        byte[] GetMeshDatas(string path)
        {
            ...
            else
            {
                ...
                if (skinned == null)
                {
                    Debug.Log("not find the element fbx file:" + path);
        ...
        byte[] GetZipDatas(string path)
        {
            ...
            string[] refrence = AssetDatabase.GetDependencies(path);
            string texture_path = refrence.Where(x => x.EndsWith(".png")).FirstOrDefault();
            string fbx_path = refrence.Where(x => x.EndsWith(".fbx")).FirstOrDefault();
            if (string.IsNullOrEmpty(texture_path) || string.IsNullOrEmpty(fbx_path))
            {
                Debug.Log($"not find the element fbx:{fbx_path} or texture:{texture_path}");
                return Array.Empty<byte>();
            }
